//***********************************************************
//	a3gim2.cpp -- Arduino Control Library for 3GIM(V2) for Mega/Leonardo..
//
//	History:
...
// Macros
#ifdef DEBUG
#  define DEBUG_PRINT(m,v)	{ SerialUSB.print("** "); SerialUSB.print((m)); SerialUSB.print(":"); SerialUSB.println((v)); }
#  define DEBUGP(m)         SerialUSB.print((m))
#else
...
#define	ISDIGIT(c)					((c) >= '0' && (c) <= '9')
	// IEM Version
#define	MIN_IEM_VERSION		3.0			// A necessary minimum IEM version of this library
	// Misc.
...
//--
	...
	if (encode == a3gsCS_UNICODE)
		return 2;	// Bad encode --@Not support R4.0
	else
		sendCommand("ASCII");
...
//		no : index number of sms slot(1..a3gsMAX_SMS_SLOT)
//	@note
//		Use gWorkBuffer and destory it
//		Change at R4.0 for 3GIM(V2)
//***************************
int A3GS::readSMS(char* msg, int msglength, char* number, int nlength, int no)
{
	int	length = sizeof(gWorkBuffer);
	char *cp;
//--
	if (no < 1 || no > a3gsMAX_SMS_SLOT)
		return 2;	// NG --@add R4.0
	
	if (no == 1)
		sendCommand("$SR");		// Send "Read SMS" command
	else {
		sprintf(gWorkBuffer, "$SR %d", no);
		sendCommand(gWorkBuffer);	// Send "Read SMS" command with index
	}
