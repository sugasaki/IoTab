//***********************************************************
//	a3gim2.h -- Arduino Control Library for 3GIM(V2) for Mega/Leonardo..
//
//	History:
...
//		** 2017.03.11 Custamized for IoT-COMB
//
//	Author:
...
/*
	Define constants
*/
//	define a3gSerial as hardware serial port
...
//	Basic constants
#define	a3gsBAUDRATE				115200	// Default UART baudrate if "a3gsBAUDRATE" is undefined(3GIM default baudrate is 9600)
...
#define	a3gsMAX_URL_LENGTH			192		// Maximum length of URL(used in httpGET() and httpPOST())
...
class A3GS
{
  public:
	...
	int readSMS(char* msg, int msglength, char* number, int nlength, int no = 1);	//@chnage R4.0
	...
	int onSMSReceived(void (*handler)(void));   //--@R3.0 no operation
...
//	int put(int storageNum, uint8_t *buffer, size_t sz);	//@delete R4.0
//	int get(int storageNum, uint8_t *buffer, size_t sz);	//@delete R4.0
		//--@R3.1 add formally two functions:	
//	int updateProfile(const uint8_t *encryptedProfile, int sz, const char *password);	//@delete R4.0
//	int encryptProfile(const char *password, const char *s, uint8_t *encryptedProfile, int *sz);	//@delete R4.0
		//--@R4.0 add function
	int enterAT(uint32_t duration);		//@add R4.0
	int getResult(char *buf, int *len, uint32_t timeout);
		//@change R4.0(move to public function)
